...
export type BookingStatusFilter = 'all' | 'pending_payment' | 'payment_processing' | 'wait_for_admin_review' | 'confirmed' | 'expired' | 'cancelled';
...
const TABS: { label: string; value: BookingStatusFilter }[] = [
  ...
  { label: 'Admin Review', value: 'wait_for_admin_review' },
    ...
    <div className="flex flex-wrap items-center gap-2 border-b border-border/40 pb-4">
      {TABS.map((tab) => (
        <button
          ...
          className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${activeStatus === tab.value
